...
COPY --from=python /source/deploy/templates /etc/foundry/legal_templates
...
ENV FOUNDRY_LEGAL_TEMPLATES_DIRECTORY "/etc/foundry/legal_templates"
...
RUN add-apt-repository --yes ppa:deadsnakes/ppa \
 ...
 && apt-get install -y libpango1.0-0 libcairo2 libpq-dev \
